...
| C5 | Spec-side duality: forward span match ⟺ backward walk | groundwork done — §6.4: `linden-reasoning/src/Equiv/SpanDuality.dfy` has `MatchesL`/`IterL` (direction-free Linden span predicate) + chain algebra, `GroupFreeL` (L1 bodies translate group-free, so the group map is constant), and `SuccActs` (walk success; gm-independent by `ResGroupMapIndep`). The two continuation-generalized directions remain: `SuccActs([Areg(r)]+cont, InputAt(str,j), Backward) ⟺ ∃i: MatchesL(r,str,i,j) ∧ SuccActs(cont, InputAt(str,i), Backward)`; then the `OB.Matches ⟺ MatchesL(Translate·)` transfer in linden-equiv via `CharSemAgree`/`AnchorSemAgree` |
