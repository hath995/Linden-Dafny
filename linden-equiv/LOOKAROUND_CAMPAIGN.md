...
| C2 (half) | Sweep completeness: every reachable `WriteOracle` gets written | in progress — §6.1: prereq `NoAccept` + closure lemma `AdvanceReachComplete` **DONE**; `FConsumeReachComplete` / `FindMatchReachComplete` (+ `ReachF` inversion) / `SweepCharacterization` open |
