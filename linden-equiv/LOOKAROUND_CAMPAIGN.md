...
| C5 | Spec-side duality: forward span match ⟺ backward walk | half done — §6.4: `SpanDuality.dfy` has `MatchesL`/`IterL` + algebra, `GroupFreeL`, `SuccActs`, the backward-walk plumbing at `InputAt` positions (`InputAtPeel`/`ReadCharBackAt`/`StrictSuffixBackAt`), nonempty chains (`IterLNE`+`IterLDropEmpty`), and the FULL completeness direction: `BwdComplete` (+`BwdCompleteQuant`/`BwdCompleteFree` companions) capped by `SpanDualityComplete` — a span match ending at cp makes the backward walk succeed. `BwdSound` (the converse, tree-side induction) remains: `SuccActs([Areg(r)]+cont, InputAt(str,j), Backward) ⟺ ∃i: MatchesL(r,str,i,j) ∧ SuccActs(cont, InputAt(str,i), Backward)`; then the `OB.Matches ⟺ MatchesL(Translate·)` transfer in linden-equiv via `CharSemAgree`/`AnchorSemAgree` |
